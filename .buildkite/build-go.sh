...
go build
mkdir dist
# removing the --- and === prefix in the output to avoid showing this as collapsible sections in BK
go test -test.v -coverprofile dist/coverage.out ./... | sed 's/^=== //' | sed 's/^--- //'
...
go run platforms/compiled/main.go
